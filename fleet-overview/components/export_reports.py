...
def generate_device_pdf_report(
    ...
    if df is not None and len(df) > 0:
        ...
        # Flagged versions section (always shown regardless of outdated count)
        flagged_versions = ['0.3.4+d8bbe0d5', '0.3.1-alpha+10c19991']
        flagged_df = df[df[column].isin(flagged_versions)]
        if not flagged_df.empty:
            pdf.add_page()
            pdf.set_font('Helvetica', 'B', 16)
            pdf.set_text_color(239, 68, 68)
            flagged_display = ', '.join(get_display_version(v) for v in flagged_versions)
            pdf.cell(0, 10, f'FLAGGED: Devices on {flagged_display}', ln=True)
            pdf.set_text_color(30, 41, 59)
            pdf.line(10, pdf.get_y(), 200, pdf.get_y())
            pdf.ln(5)
            ...
            pdf.set_font('Helvetica', '', 10)
            pdf.set_text_color(100, 116, 139)
            pdf.cell(0, 7, f'{len(flagged_df)} devices on flagged versions - requires immediate attention.', ln=True)
            pdf.ln(5)
            ...
            for region in flagged_regions:
                ...
                region_flagged = region_flagged.copy()
                region_flagged['display_version'] = region_flagged[column].apply(
